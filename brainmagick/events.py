...
class Word(Event):
    ...
    Note
    ----
    See function `brainmagick.events::extract_word_sequence_info` to compute fields `word_index` and
...
class EventAccessor:
    ...
    Alternatively, the definitions of the Event (sub)classes can be inspected in `brainmagick/events.py`.
    ...
    def _validate_event(self, event: pd.Series) -> dict:
        ...
        if event['kind'] not in self.CLASS_KIND_MAPPING.keys():
            raise ValueError(
                ...
                ' creating new `Event` classes in `brainmagick.events`.')
    ...
    def plot(self, window_s: float = 30.0, ax: tp.Optional[tp.Any] = None, show_desc: bool = True,
        ...
        See `brainmagick.viz.plot_events`.
        """
